...
public class GenerateButBruh3 : MonoBehaviour
{
    ...
    {
        var objects = Resources.FindObjectsOfTypeAll<GameObject>().Where(obj => obj.name == "door");
        GameObject [] doors = new GameObject[objects.Count()];
        doors = objects.ToArray();
        foreach (var door in objects)
        {
            doorPos.Add(door.transform.position);
        }
        foreach (var door in doors)
        {
            doorPosCount = 0;
            if (doorPos.Contains(door.transform.position)) doorPosCount++;
            if (doorPosCount == 2) DestroyImmediate(door);
        }
    }
